...
// ========== ARRAY SIZES ==========
#define MAX_CUSTOMERS 50   // ← ADD THIS LINE
#define MAX_PRODUCTS 50    // ← ADD THIS LINE (optional, good to have)
...
{
    if(dialogid == 100 && response)  // Products dialog
    {
        PlayerProduct[playerid] = listitem;
        
        // Check delivery history before showing customers
        new playerName[24];
        GetPlayerName(playerid, playerName, 24);
        
        new query[256];
        mysql_format(g_SQL, query, sizeof(query),
            "SELECT customer_id FROM delivery_history WHERE player_name = '%e' AND product_id = %d",
            playerName, PlayerProduct[playerid] + 1);
        
        mysql_tquery(g_SQL, query, "CheckDeliveryHistory", "ii", playerid, PlayerProduct[playerid]);
        return 1;
    }
    
    if(dialogid == 101 && response)  // Customers dialog
    {
    ...
    if(dialogid == 102 && response)  // Confirm dialog
    {
...
// ========== CHECK DELIVERY HISTORY CALLBACK ==========
public CheckDeliveryHistory(playerid, productIndex)
{
    new deliveredCount = cache_get_row_count();
    
    // Create array to track which customers are already served
    new bool:AlreadyDelivered[MAX_CUSTOMERS];
    ...
    // Build customer dialog (excluding already delivered)
    new dialogStr[512];
    ...
    if(availableCount == 0)
    {
        // No customers available for this product
        SendClientMessage(playerid, 0xFF0000FF, "ERROR: You've already delivered this product to all customers!");
        SendClientMessage(playerid, 0xFFFF00FF, "Choose a different product or try again later!");
    ...
    ShowPlayerDialog(playerid, 101, DIALOG_STYLE_LIST,
                    "Select Customer (Available Only)", dialogStr, "Next", "Cancel");
    return 1;
}
...
public OnPlayerEnterCheckpoint(playerid)
{
    ...
    else
    {
        ...
        // ========== ADD THIS PART (NO-REPEAT SYSTEM) ==========
        new playerName[24];
        GetPlayerName(playerid, playerName, 24);
        
        new query[256];
        mysql_format(g_SQL, query, sizeof(query),
            "INSERT INTO delivery_history (player_name, product_id, customer_id) VALUES ('%e', %d, %d)",
            playerName, PlayerProduct[playerid] + 1, PlayerCustomer[playerid] + 1);
